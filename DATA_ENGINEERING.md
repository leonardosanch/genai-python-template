...
## SP to PySpark Migration

### Pattern

Stored procedures are migrated to `SparkPipeline` subclasses following the ETL pattern:

| SP Concept | PySpark Equivalent |
|---|---|
| `FROM / source tables` | `extract()` — JDBC read via `spark.read.format("jdbc")` |
| `Business logic (filters, joins, aggregations)` | `transform()` — DataFrame operations |
| `INSERT INTO / output` | `load()` — Write to Parquet/Delta/JDBC |

### Architecture

```
SparkPipeline (abstract)          # src/application/pipelines/spark_pipeline.py
├── extract() -> DataFrame
├── transform(df) -> DataFrame
├── load(df) -> int
├── run() -> PipelineResult       # sync, Template Method
└── run_async() -> PipelineResult  # asyncio.to_thread wrapper
```

### Key Decisions

- **DataFrame-native hierarchy**: Separate from `Pipeline` (dict-based) to avoid `collect()` which kills distributed performance
- **Singleton SparkSession**: `SparkSessionManager` — thread-safe, lazy, reused across pipelines
- **Async compliance**: All adapters use `asyncio.to_thread()` since PySpark is blocking
- **Optional dependency**: Lazy imports + `TYPE_CHECKING` guard — template works without PySpark installed

### Configuration

```env
SPARK__APP_NAME=my-migration
SPARK__MASTER=yarn
SPARK__EXECUTOR_MEMORY=4g
SPARK__EXECUTOR_CORES=4
SPARK__DELTA_ENABLED=true
```

### Example

See `src/application/pipelines/sp_migration_example.py` for a complete reference implementation.

### Infrastructure Adapters

| Adapter | Port | Purpose |
|---|---|---|
| `SparkJDBCSource` | `DataSourcePort` | Read from JDBC (parallel reads supported) |
| `SparkFileSource` | `DataSourcePort` | Read Parquet/CSV/JSON/Delta |
| `SparkFileSink` | `DataSinkPort` | Write Parquet/CSV/JSON/Delta |
| `SparkJDBCSink` | `DataSinkPort` | Write to JDBC |

### API

- `POST /api/v1/spark/run` — Execute pipeline synchronously (via thread)
- `POST /api/v1/spark/run/async` — Submit to Celery for background execution
