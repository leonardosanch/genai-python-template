...
Answer:"""
        ...
        # Extract sources
        sources = [meta.get("doc_id", "unknown") for meta in (results["metadatas"][0] or [])]
...
async def health_check() -> HealthResponse:
    ...
    return HealthResponse(
        status="healthy",
        documents_indexed=app.state.rag.get_document_count(),
        version="1.0.0",
    )
...
async def index_document(document: Document):
    ...
    try:
        chunks_created = await app.state.rag.index_document(
            doc_id=doc_id,
...
async def query_rag(request: QueryRequest):
    ...
    try:
        answer, sources, tokens, cost = await app.state.rag.query(
...
async def stream_query(request: QueryRequest):
    ...
    async def generate():
        try:
            async for chunk in app.state.rag.stream_query(
                question=request.question,
                top_k=request.top_k,
            ):
                yield f"data: {chunk}\n\n"
