...
class ChromaDBStore(VectorStore):
    ...
    async def add(self, documents: list[Document]) -> None:
        """Add documents."""
        # Filter out documents without embeddings, as ChromaDB expects non-None embeddings
        ...
        if docs_with_embeddings:
            self.collection.add(
                ...
                embeddings=cast(Any, [doc.embedding for doc in docs_with_embeddings]),
    ...
    async def search(self, query_embedding: list[float], top_k: int) -> list[Document]:
        ...
        if (
            ...
            for i, doc_id in enumerate(results["ids"][0]):
                ...
                # Ensure metadata values are strings if not already
...
async def benchmark_store(store: VectorStore, name: str, embedder: "Embedder") -> dict[str, float]:
