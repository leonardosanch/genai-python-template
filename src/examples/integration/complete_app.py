...
class RAGSystem:
    ...
    async def query(self, question: str, top_k: int = 3) -> QueryResponse:
        ...
        if results["metadatas"] and results["metadatas"][0]:
            for meta_item in results["metadatas"][0]:
                if meta_item:
                    sources.append(str(meta_item.get("filename", "unknown")))
...
async def add_document(doc: Document) -> dict[str, str]:
    """Add document."""
    rag: RAGSystem = app.state.rag
    doc_id = await rag.add_document(doc.content, doc.metadata)
...
async def query(request: QueryRequest) -> QueryResponse:
    """Query system."""
    rag: RAGSystem = app.state.rag
    return await rag.query(request.question, request.top_k)
