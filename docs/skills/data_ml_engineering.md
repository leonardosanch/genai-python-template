...
- Pipelines MUST be idempotent — use upsert patterns, never insert without uniqueness checks
- Polars over Pandas for datasets >1GB — 10-100x performance improvement with lazy evaluation
- MLflow tracking MANDATORY for all experiments — track params, metrics, and models every run
- Schema validation at ingestion point — use Pydantic for APIs, Pandera for DataFrames, Great Expectations for pipelines
- Versioned migrations only (Alembic or Django migrations) — NEVER `metadata.create_all()` in production

**Read full skill when:** Building ETL pipelines, choosing data processing frameworks, setting up experiment tracking, implementing data quality checks, or migrating from stored procedures to PySpark.

---

## Versiones y Breaking Changes

| Dependencia | Versión Mínima | Breaking Changes |
|-------------|----------------|------------------|
| polars | >= 1.0.0 | `.apply()` → `.map_elements()` |
| pandas | >= 2.0.0 | `append()` eliminado |
| great-expectations | >= 0.18.0 | Clases renombradas |
| mlflow | >= 2.10.0 | API de tracking actualizada |
| dvc | >= 3.0.0 | Comandos CLI cambiados |

### Polars 1.0 Migration

```python
# ❌ Deprecated en Polars 1.0
df = df.with_columns(pl.col("x").apply(lambda x: x * 2))

# ✅ Correcto en Polars 1.0
df = df.with_columns(pl.col("x").map_elements(lambda x: x * 2, return_dtype=pl.Int64))

# ✅ Mejor: usar expresiones nativas (más rápido)
df = df.with_columns((pl.col("x") * 2).alias("x"))
```
...
### Decision Tree 1: Pandas vs Polars vs PySpark

```
¿Qué tamaño tiene tu dataset?
├── < 1GB (cabe en RAM cómodamente)
│   └── ¿Necesitas compatibilidad con librerías que solo aceptan Pandas?
│       ├── SÍ → Pandas
│       └── NO → Polars (10-100x más rápido, lazy evaluation)
├── 1GB - 50GB (cabe en RAM con optimización)
│   └── Polars con lazy evaluation
│       ├── scan_parquet() → lazy → collect()
│       ├── No cargar todo en RAM de una vez
│       └── Si no alcanza: Polars streaming o DuckDB
├── 50GB+ (no cabe en RAM)
│   └── ¿Necesitas procesamiento distribuido?
│       ├── SÍ → PySpark (cluster)
│       └── NO → DuckDB (OLAP local, out-of-core)
└── TB+ scale
    └── PySpark en cluster (Databricks, EMR, Dataproc)
```

### Decision Tree 2: Qué orchestrator usar

```
¿Cuál es tu escenario?
├── DAGs complejos con scheduling, ya hay infra Airflow
│   └── Apache Airflow
│       ├── Maduro, ecosystem grande
│       └── Verbose, requiere infra dedicada
├── Workflows Python-native, dynamic DAGs
│   └── Prefect
│       ├── Decorators simples (@flow, @task)
│       ├── Mejor DX que Airflow
│       └── Cloud o self-hosted
├── Asset-based (orientado a datos, no a tareas)
│   └── Dagster
│       ├── Software-defined assets
│       ├── Data quality nativo
│       └── Mejor para data platforms
└── Simple ETL, equipo pequeño
    └── Celery + scheduled tasks
        └── No necesitas un orchestrator completo
```

### Decision Tree 3: Classical ML vs LLM

```
¿Qué tipo de problema tienes?
├── Clasificación/regresión con features tabulares
│   └── Classical ML (scikit-learn, XGBoost)
│       ├── Más barato, más rápido, más predecible
│       └── Fine si tienes dataset etiquetado
├── NLP: clasificación de texto, sentiment, NER
│   └── ¿Tienes dataset etiquetado > 1000 examples?
│       ├── SÍ → Fine-tuned BERT/classifier (más barato en producción)
│       └── NO → LLM con few-shot o zero-shot
├── Generación de texto libre
│   └── LLM (siempre)
├── Q&A sobre documentos
│   └── RAG (ver genai_rag skill)
└── Mixto (clasificar + generar)
    └── Hybrid: ML classifier para routing + LLM para generación
        └── Clasificador decide si necesita LLM → ahorra costos
```
...
### SP to PySpark Migration Scaffolding
- **SparkPipeline base class**: `src/application/pipelines/spark_pipeline.py` — DataFrame-native ETL base
- **SparkSessionManager**: `src/infrastructure/data/spark/session_manager.py` — Thread-safe singleton
- **JDBC/File adapters**: `src/infrastructure/data/spark/` — Port-compliant adapters with async wrappers
- **Migration example**: `src/application/pipelines/sp_migration_example.py` — Reference pattern
- See [DATA_ENGINEERING.md](../../DATA_ENGINEERING.md) for full documentation
