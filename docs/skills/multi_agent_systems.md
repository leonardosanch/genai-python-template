...
**Critical multi-agent rules (enforce always):**
- Each agent has ONE clear responsibility — never build "god agents"
- EVERY agent loop has max_iterations configured (default: 10) — unbounded loops cause infinite execution
- LangGraph checkpointing enabled for all stateful workflows — enables recovery and time-travel debugging
- Human-in-the-loop required for critical decisions — never fully automate high-impact actions
- LangSmith tracing mandatory for all multi-agent systems — observe agent decisions and costs

**Read full skill when:** Designing multi-agent coordination patterns, implementing LangGraph workflows, debugging agent loops, setting up human approval gates, or tracking multi-agent costs.

---

## Versiones y Advertencias de Frameworks

| Framework | Versión Mínima | Estabilidad | Notas |
|-----------|----------------|-------------|-------|
| langgraph | >= 0.2.0 | ⚠️ API estabilizándose | Verificar changelog |
| crewai | >= 0.30.0 | ⚠️ Cambios frecuentes | Verificar docs |
| autogen | >= 0.2.0 | ⚠️ En desarrollo activo | Microsoft puede renombrar |

> ⚠️ **LangGraph**: El paquete `langgraph.prebuilt` puede no existir en versiones anteriores a 0.2.0. Verificar imports.

### Timeout Global para Workflows
...
async def run_workflow_with_timeout(
    ...
    """Ejecutar workflow con timeout global para evitar loops infinitos."""
    try:
        return await asyncio.wait_for(
            workflow.ainvoke(input_data),
            timeout=timeout_seconds,
        )
    except asyncio.TimeoutError:
        raise RuntimeError(
            f"Workflow excedió timeout de {timeout_seconds}s. "
            "Verificar bounded loops y condiciones de salida."
        )
```
...
# Para workflows muy profundos, considerar aumentar límite de recursión
# DEFAULT: 1000, aumentar solo si es necesario
# sys.setrecursionlimit(2000)  # Usar con precaución

# Mejor: usar iteración con bounded loops
MAX_AGENT_ITERATIONS = 25  # Límite absoluto
```
...
### Decision Tree 1: ¿Necesitas un sistema multi-agent?

```
¿Cuántas responsabilidades tiene tu agente?
├── 1 responsabilidad clara
│   └── Single agent (NO multi-agent)
│       └── Agregar complejidad sin necesidad es el error #1
├── 2-3 responsabilidades bien definidas
│   └── ¿Las responsabilidades son secuenciales?
│       ├── SÍ → Pipeline (agente 1 → agente 2 → agente 3)
│       └── NO → Supervisor con 2-3 workers
└── 4+ responsabilidades o routing dinámico
    └── Multi-agent con patrón apropiado (ver Decision Tree 2)
```

### Decision Tree 2: Qué patrón de coordinación usar

```
¿Cómo se distribuye el trabajo?
├── Un agente decide quién trabaja
│   └── Supervisor pattern
│       ├── El supervisor rutea, no ejecuta
│       ├── Workers especializados
│       └── Ideal para: customer support, task routing
├── Pipeline fijo paso a paso
│   └── Sequential pattern
│       ├── Cada agente procesa y pasa al siguiente
│       └── Ideal para: document processing, review chains
├── Subtareas jerárquicas
│   └── Hierarchical pattern
│       ├── Supervisor → sub-supervisors → workers
│       └── Ideal para: research, complex planning
├── Agentes discuten y votan
│   └── Collaborative/Debate pattern
│       ├── Múltiples perspectivas
│       └── Ideal para: decision making, code review
└── El agente decide dinámicamente a quién handoff
    └── Swarm pattern
        ├── Sin coordinador central
        └── Ideal para: adaptive workflows, emergent behavior
```

### Decision Tree 3: Qué framework usar

```
¿Cuál es tu prioridad?
├── Control fino sobre estado y flujo
│   └── LangGraph (primary choice)
│       ├── State machines, checkpointing, human-in-the-loop
│       └── Más código, máximo control
├── Equipos de agentes con roles claros
│   └── CrewAI
│       ├── Role-based, task delegation, tools
│       └── Menos código, menos control
├── Agentes conversacionales multi-turn
│   └── AutoGen
│       ├── Conversational, debate, code execution
│       └── Bueno para prototipado rápido
└── Integración enterprise (.NET/Java/Python)
    └── Semantic Kernel
        └── Multi-language, Azure native
```
